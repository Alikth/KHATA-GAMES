(() => {
  ...
  async function load(){try{var d=await api('/api/my-castle/assets');render(d);await loadWarCommands(d.castle);}catch(e){$('castleManagementRoot').innerHTML='<div class="cm-error">❌ '+esc(e.message)+'</div>';}}
  async function loadWarCommands(castle){
    try {
      const [arrived, active] = await Promise.all([
        api('/api/my-war-expeditions/commands'),
      ...
      root.innerHTML = activeHtml + arrivedHtml + (activeHtml || arrivedHtml ? '' : '<div class="cm-placeholder">لشکرکشی فعالی وجود ندارد.</div>');
    } catch (e) {
      $('cmWarCommands').innerHTML = '<div class="cm-error">❌ ' + esc(e.message) + '</div>';
    }
  }
