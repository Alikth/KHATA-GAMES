window.addEventListener("DOMContentLoaded", () => {
  ...
  async function renderMyCastles() {
    ...
    root.innerHTML = mine.map(p => {
      ...
      return `<article class="my-castle-card"><div class="my-castle-art">${escapeHTML(c?.icon || "🏰")}</div><div class="my-castle-body"><span class="my-castle-region">${escapeHTML(r?.icon || "")} ${escapeHTML(p.region)}</span><h3>${escapeHTML(p.castle)}</h3><p>HOUSE ${escapeHTML(p.house)}</p><div class="my-castle-meta"><span>👤 ${escapeHTML(p.username)}</span><span class="owned-badge">YOUR CASTLE</span></div></div><button class="castle-open" type="button" data-action="castle" data-region="${houses.findIndex(x => x.region === p.region)}" data-castle="${r?.castles.findIndex(x => x.castle === p.castle) ?? 0}">مشاهده جزئیات</button></article>`;
    }).join("");
  }
